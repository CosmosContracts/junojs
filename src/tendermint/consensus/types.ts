...
import { BitArray, type BitArrayAmino } from "../libs/bits/types"
import {
	BlockID,
	type BlockIDAmino,
	Part,
...
import type { ProposalAmino as Proposal1Amino } from "../types/types"
import { Vote as Vote1 } from "../types/types"
import type { VoteAmino as Vote1Amino } from "../types/types"
/**
...
export const NewRoundStep = {
	...
	fromPartial(object: Partial<NewRoundStep>): NewRoundStep {
		...
		message.secondsSinceStartTime =
			object.secondsSinceStartTime !== undefined && object.secondsSinceStartTime !== null
				? BigInt(object.secondsSinceStartTime.toString())
				: BigInt(0)
	...
	fromAmino(object: NewRoundStepAmino): NewRoundStep {
		...
		if (
			object.seconds_since_start_time !== undefined &&
			object.seconds_since_start_time !== null
		) {
	...
	toAmino(message: NewRoundStep): NewRoundStepAmino {
		...
		obj.seconds_since_start_time =
			message.secondsSinceStartTime !== BigInt(0)
				? message.secondsSinceStartTime?.toString()
				: undefined
...
export const NewValidBlock = {
	...
	toAmino(message: NewValidBlock): NewValidBlockAmino {
		...
		obj.height = message.height !== BigInt(0) ? message.height?.toString() : undefined
		obj.round = message.round === 0 ? undefined : message.round
		obj.block_part_set_header = message.blockPartSetHeader
			? PartSetHeader.toAmino(message.blockPartSetHeader)
			: undefined
...
export const Message = {
	...
	toAmino(message: Message): MessageAmino {
		const obj: any = {}
		obj.new_round_step = message.newRoundStep
			? NewRoundStep.toAmino(message.newRoundStep)
			: undefined
		obj.new_valid_block = message.newValidBlock
			? NewValidBlock.toAmino(message.newValidBlock)
			: undefined
		...
		obj.vote_set_maj23 = message.voteSetMaj23
			? VoteSetMaj23.toAmino(message.voteSetMaj23)
			: undefined
